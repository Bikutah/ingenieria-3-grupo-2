...
import { Ban, BadgeDollarSign, XCircle, Check, ChevronsUpDown, Info } from "lucide-react";
...
function ComandaDetalleDialog({
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const data = await comandasService.getById(comandaId);
                if (!mounted) return;
                setComanda(data);
            } catch (e: any) {
                if (!mounted) return;
